...
use astronomicon_core::units::{Duration, Mass, Position, Quaternion, Vector3, VelocityVector};
use astronomicon_db::SqlitePool;
use rocketcon_core::domain::{
    ComponentDetails, ComponentPayloadState, PayloadDeploymentEvent, VehiclePhysicalState,
};
...
fn rotate_vector_by_quaternion(q: Quaternion, v: Vector3) -> Vector3 {
    let u = Vector3::new(q.x(), q.y(), q.z());
    let w = q.w();
    let cross_uv = u.cross(&v);
    let term = v * w + cross_uv;
    v + u.cross(&term) * 2.0
}
...
pub async fn deploy_payload(
    ...
    let mother_state = vehicle_physical_state_repository::get_by_vehicle_id(pool, &mother_vehicle_id)
        .await?
        .ok_or_else(|| {
    ...
    let components = vehicle_repository::list_components_for_vehicle(pool, &mother_vehicle_id).await?;
    ...
    let (entry, record) = components
        ...
        .ok_or_else(|| {
            RocketError::Generic(format!(
                ...
                vehicle_component_id, mother_vehicle_id
            ))
        })?;
    ...
    let payload_spec = match record.details() {
        ...
        _ => {
            return Err(RocketError::Generic(format!(
                "component '{}' is not a payload dispenser or fairing",
                vehicle_component_id
            )));
        }
    };

    let already_deployed = payload_state_repository::is_deployed(pool, &vehicle_component_id)
        .await?
        .unwrap_or(false);

    if already_deployed {
        return Err(RocketError::Generic(format!(
            "payload for component '{}' is already deployed",
            vehicle_component_id
        )));
    }

    let ejected_mass = if let Some(contained_id) = payload_spec.contained_vehicle_id() {
        crate::aeroespacial::resolve_vehicle_real_mass(pool, contained_id, universe_epoch, at_epoch).await?
    ...
    let rotated_mount_offset = rotate_vector_by_quaternion(mother_state.orientation(), mount_offset);
    ...
    let global_directional_vector = rotate_vector_by_quaternion(mother_state.orientation(), local_axis).normalized();

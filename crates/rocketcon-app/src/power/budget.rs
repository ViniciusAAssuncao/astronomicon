...
use crate::power::battery::resolve_vehicle_battery_state;
...
pub async fn resolve_vehicle_power_budget(
    ...
    let mut contributions: Vec<ComponentPowerContribution> = Vec::with_capacity(components.len());
    ...
    for (entry, record) in &components {
        ...
        contributions.push(ComponentPowerContribution::new(
    ...
    let (stored_energy, battery_capacity) =
        resolve_vehicle_battery_state(pool, vehicle_id, universe_epoch, at_epoch).await?;

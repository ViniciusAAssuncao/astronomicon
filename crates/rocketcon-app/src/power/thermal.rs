...
use rocketcon_core::math::aerothermodynamics::{
    ...
    vehicle_geometry_thermal_properties,
};
...
impl VehicleThermalBudget {
    ...
    pub fn new_simple(
        ...
        Self {
            ...
            effective_radiator_ga_product,
            equilibrium_temperature,
            max_allowable_temperature: Temperature::new(DEFAULT_MAX_STRUCTURAL_TEMPERATURE_K),
            is_overheating: equilibrium_temperature.value() > DEFAULT_MAX_STRUCTURAL_TEMPERATURE_K,
        }
    }
}
...
pub async fn resolve_vehicle_thermal_budget(
    ...
    let (aero_power, stag_flux, skin_flux, hull_area) = match aero_results {
        ...
        None => {
            let (_, nose_area, side_area) = vehicle_geometry_thermal_properties(
                &components,
                &stages
            );
            (Luminosity::new(0.0), HeatFlux::new(0.0), HeatFlux::new(0.0), nose_area + side_area)
        }
    };
    ...
    let max_temp = Temperature::new(DEFAULT_MAX_STRUCTURAL_TEMPERATURE_K);

    check_thermal_structural_limits(eq_temp, max_temp)?;

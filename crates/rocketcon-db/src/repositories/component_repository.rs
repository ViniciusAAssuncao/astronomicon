...
use crate::models::{
    ComponentBatteryRow,
    ComponentEngineRow,
    ComponentPropellantTankRow,
    ComponentReactionControlThrusterRow,
    ComponentReactionWheelRow,
    ComponentRow,
...
async fn fetch_component_details(
    ...
    match component.kind() {
        ...
        ComponentKind::PropellantTank => {
            let row = sqlx
                ::query_as::<_, ComponentPropellantTankRow>(
        ...
        ComponentKind::Battery => {
            let row = sqlx
                ::query_as::<_, ComponentBatteryRow>(
        ...
        ComponentKind::SolarPanel => {
            let row = sqlx
                ::query_as::<_, ComponentSolarPanelRow>(
        ...
        ComponentKind::ReactionControlThruster => {
            let row = sqlx
                ::query_as::<_, ComponentReactionControlThrusterRow>(
                    "SELECT component_id, propellant_id, specific_impulse_vacuum_s, max_thrust_n, min_impulse_bit_n_s FROM component_reaction_control_thrusters WHERE component_id = ?"
                )
                .bind(&id_str)
                .fetch_optional(pool).await?
                .ok_or_else(|| {
                    RocketDbError::Domain(RocketDomainError::InvalidInvariant {
                        field: "component_reaction_control_thrusters".to_string(),
                        reason: format!(
                            "missing reaction control thruster specification for component id '{}'",
                            component.id()
                        ),
                    })
                })?;

            let spec = ReactionControlThrusterSpecification::try_from(row)?;
            Ok(ComponentDetails::ReactionControlThruster(spec))
        }
        ComponentKind::ReactionWheel => {
            let row = sqlx
                ::query_as::<_, ComponentReactionWheelRow>(
                    "SELECT component_id, max_torque_n_m, max_angular_momentum_storage_n_m_s FROM component_reaction_wheels WHERE component_id = ?"
                )
                .bind(&id_str)
                .fetch_optional(pool).await?
                .ok_or_else(|| {
                    RocketDbError::Domain(RocketDomainError::InvalidInvariant {
                        field: "component_reaction_wheels".to_string(),
                        reason: format!(
                            "missing reaction wheel specification for component id '{}'",
                            component.id()
                        ),
                    })
                })?;

            let spec = ReactionWheelSpecification::try_from(row)?;
...
pub async fn list_all(pool: &SqlitePool) -> RocketDbResult<Vec<ComponentRecord>> {
    ...
    let rows = sqlx::query_as::<_, ComponentRow>(&query).fetch_all(pool).await?;

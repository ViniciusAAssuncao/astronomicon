...
use crate::models::TrajectoryPatchRow;
use astronomicon_core::units::Duration;
use rocketcon_core::domain::TrajectoryPatch;
use sqlx::SqlitePool;
use uuid::Uuid;

const BASE_QUERY: &str = "SELECT id, vehicle_id, reference_body_id, start_universe_epoch_s, end_universe_epoch_s, semi_major_axis_m, eccentricity, inclination_rad, longitude_of_ascending_node_rad, argument_of_periapsis_rad, true_anomaly_at_epoch_rad, gravitational_parameter_m3_s2 FROM vehicle_trajectory_patches";
...
pub async fn insert_patch(pool: &SqlitePool, patch: &TrajectoryPatch) -> RocketDbResult<()> {
    sqlx::query(
        "INSERT INTO vehicle_trajectory_patches (
            ...
            true_anomaly_at_epoch_rad, gravitational_parameter_m3_s2
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    )
    ...
    .bind(patch.semi_major_axis().value())
    .bind(patch.eccentricity())
    .bind(patch.inclination().value())
    .bind(patch.longitude_of_ascending_node().value())
    .bind(patch.argument_of_periapsis().value())
    .bind(patch.true_anomaly_at_epoch().value())

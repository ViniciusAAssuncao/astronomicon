...
const BASE_QUERY: &str = "SELECT vehicle_component_id, load_fraction, captured_universe_epoch_s, captured_at_epoch_s FROM component_operational_states";
...
pub async fn upsert(
    ...
    sqlx::query(
        "INSERT INTO component_operational_states (vehicle_component_id, load_fraction, captured_universe_epoch_s, captured_at_epoch_s) VALUES (?, ?, ?, ?) ON CONFLICT(vehicle_component_id) DO UPDATE SET load_fraction = excluded.load_fraction, captured_universe_epoch_s = excluded.captured_universe_epoch_s, captured_at_epoch_s = excluded.captured_at_epoch_s",
    )

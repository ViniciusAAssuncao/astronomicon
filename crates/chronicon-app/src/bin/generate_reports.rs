use astronomicon_core::units::constants::{ASTRONOMICAL_UNIT, SECONDS_PER_YEAR};
...
fn build_full_markdown_report(
    ...
    md.push_str(&format!("| **Duração Base do Dia Adotada** | {:.4} s ({:.4} h) |\n", resolved.day_duration.value(), resolved.day_duration.value() / 3600.0));
    md.push_str(&format!("| **Duração Base do Ano Adotada** | {:.4} s ({:.4} dias padrão) |\n", resolved.year_duration.value(), resolved.year_duration.value() / 86400.0));
    ...
    md.push_str(&format!(
        ...
        skel.day_info
            .sidereal_day()
            .map(|d| format!("{:.4} s ({:.4} h)", d.value(), d.value() / 3600.0))
    ...
    md.push_str(&format!(
        ...
        skel.day_info
            .orbital_period()
            .map(|d| format!("{:.4} s ({:.4} dias padrão)", d.value(), d.value() / 86400.0))
            .unwrap_or_else(|| "Indisponível".to_string())
    ));
    ...
    let prec_arcsec_yr = prec_rate * (180.0 / std::f64::consts::PI) * 3600.0 * SECONDS_PER_YEAR;
    md.push_str(&format!("| **Taxa de Precessão Axial** | {:.8e} rad/s ({:.4} arcsec/ano) |\n", prec_rate, prec_arcsec_yr));
    md.push_str(&format!(
        "| **Período de Precessão Axial** | {} |\n",
        skel.year_info
            .axial_precession_period()
            .map(|d| format!("{:.2} anos padrão ({:.4e} s)", d.value() / SECONDS_PER_YEAR, d.value()))
    ...
    md.push_str(&format!("| **Período Orbital de Referência** | {:.4} s |\n\n", sea.orbital_period().value()));
    ...
    if let Some(cp) = sea.cardinal_points() {
        ...
        let t_day = resolved.day_duration.value().max(1e-6);
        md.push_str(&format!(
            ...
            cp.north_spring_equinox_time.value() / t_day
        ));
        md.push_str(&format!(
            ...
            cp.north_summer_solstice_time.value() / t_day
        ));
        md.push_str(&format!(
            ...
            cp.north_autumn_equinox_time.value() / t_day
        ));
        md.push_str(&format!(
            ...
            cp.north_winter_solstice_time.value() / t_day
        ));
    }
    ...
    if let (Some(nd), Some(sd)) = (sea.northern_hemisphere_durations(), sea.southern_hemisphere_durations()) {
        ...
        md.push_str(&format!(
            "| **Primavera** | {:.2} dias ({:.2} s) | {:.2} dias ({:.2} s) |\n",
            nd.spring.value() / t_day, nd.spring.value(),
            sd.spring.value() / t_day, sd.spring.value()
        ));
        md.push_str(&format!(
            "| **Verão** | {:.2} dias ({:.2} s) | {:.2} dias ({:.2} s) |\n",
            nd.summer.value() / t_day, nd.summer.value(),
            sd.summer.value() / t_day, sd.summer.value()
        ));
        md.push_str(&format!(
            "| **Outono** | {:.2} dias ({:.2} s) | {:.2} dias ({:.2} s) |\n",
            nd.autumn.value() / t_day, nd.autumn.value(),
            sd.autumn.value() / t_day, sd.autumn.value()
        ));
        md.push_str(&format!(
            "| **Inverno** | {:.2} dias ({:.2} s) | {:.2} dias ({:.2} s) |\n\n",
            nd.winter.value() / t_day, nd.winter.value(),
            sd.winter.value() / t_day, sd.winter.value()
        ));
    }
    ...
    if moon_sys.moons().is_empty() {
        ...
        for moon in moon_sys.moons() {
            ...
            let sid = moon.sidereal_month().map(|d| format!("{:.4} d", d.value() / 86400.0)).unwrap_or_else(|| "N/A".to_string());
            let syn = moon.synodic_month().map(|d| format!("{:.4} d", d.value() / 86400.0)).unwrap_or_else(|| "N/A".to_string());
            let anom = moon.anomalistic_month().map(|d| format!("{:.4} d", d.value() / 86400.0)).unwrap_or_else(|| "N/A".to_string());
            let drac = moon.draconic_month().map(|d| format!("{:.4} d", d.value() / 86400.0)).unwrap_or_else(|| "N/A".to_string());
        ...
        for moon in moon_sys.moons() {
            if let Some(saros) = moon.saros_cycle() {
                md.push_str(&format!(
                    "| **{}** | {} | {} | {} | {:.2} dias ({:.2} anos) | {:.2} h | {:.2} h |\n",
                    moon.moon_name(),
                    saros.synodic_month_count,
                    saros.draconic_month_count,
                    saros.anomalistic_month_count,
                    saros.duration.value() / 86400.0,
                    saros.duration.value() / SECONDS_PER_YEAR,
                    saros.synodic_draconic_discrepancy.value() / 3600.0,
                    saros.synodic_anomalistic_discrepancy.value() / 3600.0
                ));
            } else {
    ...
    md.push_str(&format!(
        ...
        poly.grand_cycle()
            .map(|d| format!("{:.4} s ({:.4} dias locais)", d.value(), d.value() / resolved.day_duration.value().max(1e-6)))
    ...
    for comp in poly.components() {
        ...
        let day_str = comp
            ...
            .map(|d| format!("{:.4} h", d.value() / 3600.0))

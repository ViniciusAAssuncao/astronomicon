...
mod tests {
    ...
    #[test]
    fn test_collinear_solvers_equal_mass() {
        let gamma1 = solve_l1_gamma(0.5).unwrap();
        assert!((gamma1 - 0.5).abs() < 1e-9);

        let gamma2 = solve_l2_gamma(0.5).unwrap();
        let gamma3 = solve_l3_gamma(0.5).unwrap();
        assert!((gamma2 - gamma3).abs() < 1e-9);
    }

    #[test]
    fn test_collinear_solvers_sun_earth() {
        let mu = 5.9722e24 / (1.98847e30 + 5.9722e24);
        let gamma1 = solve_l1_gamma(mu).unwrap();
        let gamma2 = solve_l2_gamma(mu).unwrap();
        let gamma3 = solve_l3_gamma(mu).unwrap();

        assert!((gamma1 - 0.010008).abs() < 1e-4);
        assert!((gamma2 - 0.010074).abs() < 1e-4);
        assert!((gamma3 - 0.999998).abs() < 1e-4);
    }

    #[test]
    fn test_lagrange_point_positions() {
        let p1 = Position::from_components(0.0, 0.0, 0.0);
        let p2 = Position::from_components(1.5e11, 0.0, 0.0);
        let m1 = Mass::new(1.98847e30);
        let m2 = Mass::new(5.9722e24);
        let normal = Vector3::new(0.0, 0.0, 1.0);

        let pos_l1 = lagrange_point_position(LagrangePoint::L1, p1, p2, m1, m2, normal).unwrap();
        let pos_l2 = lagrange_point_position(LagrangePoint::L2, p1, p2, m1, m2, normal).unwrap();
        let pos_l3 = lagrange_point_position(LagrangePoint::L3, p1, p2, m1, m2, normal).unwrap();
        let pos_l4 = lagrange_point_position(LagrangePoint::L4, p1, p2, m1, m2, normal).unwrap();
        let pos_l5 = lagrange_point_position(LagrangePoint::L5, p1, p2, m1, m2, normal).unwrap();

        assert!(pos_l1.raw().0 > 0.0 && pos_l1.raw().0 < 1.5e11);
        assert!(pos_l2.raw().0 > 1.5e11);
        assert!(pos_l3.raw().0 < 0.0);

        let d_l4_p1 = (pos_l4 - p1).magnitude().value();
        let d_l4_p2 = (pos_l4 - p2).magnitude().value();
        assert!((d_l4_p1 - 1.5e11).abs() < 1.0);
        assert!((d_l4_p2 - 1.5e11).abs() < 1.0);

        let d_l5_p1 = (pos_l5 - p1).magnitude().value();
        let d_l5_p2 = (pos_l5 - p2).magnitude().value();
        assert!((d_l5_p1 - 1.5e11).abs() < 1.0);
        assert!((d_l5_p2 - 1.5e11).abs() < 1.0);
    }

    #[test]
    fn test_lagrange_point_stability() {
        let m1 = Mass::new(1.98847e30);
        let m2 = Mass::new(5.9722e24);

        assert!(!is_lagrange_point_stable(LagrangePoint::L1, m1, m2));
        assert!(!is_lagrange_point_stable(LagrangePoint::L2, m1, m2));
        assert!(!is_lagrange_point_stable(LagrangePoint::L3, m1, m2));
        assert!(is_lagrange_point_stable(LagrangePoint::L4, m1, m2));
        assert!(is_lagrange_point_stable(LagrangePoint::L5, m1, m2));

        let m_eq1 = Mass::new(1.0e30);
        let m_eq2 = Mass::new(1.0e30);
        assert!(!is_lagrange_point_stable(LagrangePoint::L4, m_eq1, m_eq2));
        assert!(!is_lagrange_point_stable(LagrangePoint::L5, m_eq1, m_eq2));
    }
}
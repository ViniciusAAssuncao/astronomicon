...
pub fn vehicle_geometry_thermal_properties(
    ...
    for (entry, record) in entries {
        ...
        if let ComponentDetails::Hull(_) = record.details() {
            hull_count += 1;
            let comp = record.component();
            let d = comp.diameter().value();
            let l = comp.length().value();
            let z_front = entry.mount_offset().2 + l * 0.5;
            ...
            if z_front > foremost_z && d.is_finite() && d > 0.0 {
                foremost_z = z_front;
                foremost_diameter = d;
            }
        }
    ...
    if hull_count > 0 {
        let nose_diameter = if foremost_diameter > 0.0 {
            foremost_diameter
        } else {
            max_diameter
        };
        let nose_r = (nose_diameter * 0.5).max(MIN_NOSE_RADIUS_M);
        let nose_area = PI * nose_r * nose_r;
        let side_area = total_lateral_area.max(MIN_LATERAL_AREA_M2);

        return (Length::new(nose_r), nose_area, side_area);
    }

...
pub async fn get_by_id(pool: &SqlitePool, id: &Uuid) -> DbResult<Option<StarRow>> {
    let row = sqlx::query_as::<_, StarRow>(
        "SELECT id, star_system_id, name, kind, mass_kg, radius_m, effective_temperature_k, \
         rotation_period_s, axial_tilt_rad, semi_major_axis_m, eccentricity, inclination_rad, \

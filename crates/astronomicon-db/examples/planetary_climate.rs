use astronomicon_core::domain::{
    Atmosphere, GasComponent, OrbitalElements, Planet, PlanetKind, Star, StarKind,
};
...
use astronomicon_core::units::constants::{
    ASTRONOMICAL_UNIT, GRAVITATIONAL_CONSTANT, STEFAN_BOLTZMANN_CONSTANT, UNIVERSAL_GAS_CONSTANT,
};
use astronomicon_core::units::{
    Angle, Length, Mass, Pressure, Temperature, TemperatureGradient,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mars_atmosphere = Atmosphere::new(
        ...
        vec![
            ...
            GasComponent::new("N2".to_string(), 2.60)?,
    ...
    let venus_atmosphere = Atmosphere::new(
        ...
        vec![
            GasComponent::new("CO2".to_string(), 96.50)?,
            GasComponent::new("N2".to_string(), 3.50)?,
        ],
    )?;
    ...
    println!("====================================================================================================");
    println!("                        ASTRONOMICON - RELATÓRIO DE CLIMA E ATMOSFERAS PLANETÁRIAS                  ");
    println!("                                (Validação Física das Frentes 7 e 8)                                ");
    println!("====================================================================================================");
    ...
    for fixture in fixtures {
        ...
        let surface_density = ideal_gas_density(atmosphere.surface_pressure(), molar_mass, t_global);

        println!("----------------------------------------------------------------------------------------------------");
        println!("PLANETA: {:<15} | TIPO: {:<12} | SEMI-EIXO: {:>8.4} AU", fixture.name, format!("{:?}", planet.kind()), semi_major.value() / ASTRONOMICAL_UNIT);
        println!("----------------------------------------------------------------------------------------------------");
        ...
        println!("  Obliquidade Axial    : {:>14.2}° ({:.4} rad)", obliquity.value() * 180.0 / PI, obliquity.value());
        println!("  Pressão de Superfície: {:>14.2} Pa ({:.4} bar)", atmosphere.surface_pressure().value(), atmosphere.surface_pressure().value() * 1e-5);
        println!("  Massa Molar Média    : {:>14.4} g/mol", molar_mass.value() * 1000.0);
        println!("  Composição Gasosa    : {}", atmosphere.composition().iter().map(|c| format!("{}: {:.2}%", c.formula(), c.percentage())).collect::<Vec<_>>().join(", "));
        println!();

        println!("[2] BALANÇO RADIATIVO E EFEITO ESTUFA:");
        println!("  Irradiância Orbital  : {:>14.2} W/m²", top_irradiance.value());
        println!("  T_eq Calculada       : {:>14.2} K ({:>6.2} °C) | Esperado: {:>6.1} K (Erro: {:.2}%)", t_eq.value(), t_eq.value() - 273.15, fixture.expected_teq_k, ((t_eq.value() - fixture.expected_teq_k) / fixture.expected_teq_k).abs() * 100.0);
        println!("  Efeito Estufa (ΔT)   : {:>14.2} K", atmosphere.greenhouse_effect().value());
        println!("  T_média Global Calc. : {:>14.2} K ({:>6.2} °C) | Esperado: {:>6.1} K (Erro: {:.2}%)", t_global.value(), t_global.value() - 273.15, fixture.expected_tmean_k, ((t_global.value() - fixture.expected_tmean_k) / fixture.expected_tmean_k).abs() * 100.0);
        println!("  Escala de Altura (H) : {:>14.2} km         | Esperado: {:>6.1} km (Erro: {:.2}%)", scale_h.value() * 1e-3, fixture.expected_scale_height_km, ((scale_h.value() * 1e-3 - fixture.expected_scale_height_km) / fixture.expected_scale_height_km).abs() * 100.0);
        println!("  Densidade Superfície : {:>14.4} kg/m³      | Esperado: {:>6.3} kg/m³ (Erro: {:.2}%)", surface_density.value(), fixture.expected_density_kg_m3, ((surface_density.value() - fixture.expected_density_kg_m3) / fixture.expected_density_kg_m3).abs() * 100.0);
        println!();

        println!("[3] DISTRIBUIÇÃO TÉRMICA LATITUDINAL (SOLSTÍCIO DE VERÃO NO HEMISFÉRIO NORTE):");
        println!("  {:<16} | {:<12} | {:<12} | {:<14} | {:<18} | {:<18}", "Latitude", "Declinação", "Fotoperíodo", "Fator Insolação", "T_eq Local", "T_Superfície");
        println!("  -----------------+--------------+--------------+----------------+--------------------+--------------------");
        ...
        for (lat_name, lat_rad) in sample_latitudes {
            ...
            let decl = solar_declination(obliquity, elements.argument_of_periapsis(), solstice_ta, summer_solstice_ta);
        ...
        println!("  {:<12} | {:<20} | {:<24} | {:<16} | {:<10}", "Altitude", "Temperatura", "Pressão", "Densidade", "P / P0");
        println!("  -------------+----------------------+--------------------------+------------------+-----------");
    ...
    println!("Status da Validação: MODELOS DE RADIAÇÃO, INSOLAÇÃO E ESTRUTURA ATMOSFÉRICA VALIDADOS COM SUCESSO.");
    println!("====================================================================================================");

...
impl CanonicalGeometry {
    ...
    pub fn sunset() -> Self {
        ...
        let cos_theta =
            ((88.0 * PI) / 180.0).cos() * ((85.0 * PI) / 180.0).cos() +
...
pub fn calculate_radiance_breakdown_at_geometry(
    ...
    let taus_ext = [optical_column.total_r, optical_column.total_g, optical_column.total_b];
    let taus_rayleigh = [
    ...
    let f_sols = [solar_irradiance.r, solar_irradiance.g, solar_irradiance.b];
    ...
    for i in 0..3 {
        ...
        let tau_a_sca = (omega * tau - tau_r).max(0.0);
        ...
        let tau_sca_tot = tau_r + tau_a_sca;
        let phase = if tau_sca_tot > 1e-12 {
            (tau_r * p_r + tau_a_sca * p_hg) / tau_sca_tot
        } else {
            p_r
        };
        ...
        single_rad[i] = if rad_s.is_finite() && rad_s > 0.0 { rad_s } else { 0.0 };

        let two_stream = delta_eddington_two_stream(tau, omega, g, mu0, surface_albedo);
        let rad_d = (two_stream.transmittance * mu0 * f_0) / PI;
        diffuse_rad[i] = if rad_d.is_finite() && rad_d > 0.0 { rad_d } else { 0.0 };

        let tot = single_rad[i] + diffuse_rad[i];
        total_rad[i] = if tot.is_finite() && tot > 0.0 { tot } else { 0.0 };
    }

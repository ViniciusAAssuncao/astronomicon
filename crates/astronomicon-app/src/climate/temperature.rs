...
use astronomicon_core::units::{ Angle, Duration, Irradiance, Position, Pressure, Temperature };
...
pub async fn resolve_irradiance_at_position(
    ...
    let orbital_distance = (target_position - star_pos).magnitude();
...
pub async fn resolve_top_of_atmosphere_irradiance(
    ...
    resolve_irradiance_at_position(pool, star, universe_epoch, at_epoch, planet_pos).await
}
...
pub async fn resolve_surface_albedo(
    ...
    if let Some(hydrosphere) = hydrosphere_repository::get_by_planet_id(pool, &planet_id).await? {
...
pub async fn resolve_global_mean_temperature(
    ...
    let effective_albedo = resolve_surface_albedo(pool, planet_id, universe_epoch, at_epoch).await?;
